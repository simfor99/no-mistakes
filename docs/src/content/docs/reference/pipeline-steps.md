...
- Continues its normal monitoring loop until the PR is merged, closed, declined, or the configured `ci_timeout` idle window elapses, then parks at an approval gate instead of ending the run; the GitHub-only queued-check guard can park earlier when every remaining check is explicitly queued and unstarted
- The [`ci_timeout` reference](/no-mistakes/reference/global-config/#ci_timeout) owns idle re-arming, unlimited monitoring, and fail-closed reconciliation while that gate is parked; [`ci_queued_check_attention_after`](/no-mistakes/reference/global-config/#ci_queued_check_attention_after) owns the early queue-stall guard
