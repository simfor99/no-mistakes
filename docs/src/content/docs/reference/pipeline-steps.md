...
- If `commands.test` is empty: the agent validates the change with evidence-oriented tests or manual checks, returning structured findings with severity, description, and `action` (`no-op`, `auto-fix`, `ask-user`). With a passing `commands.test`, the same optional pass runs only when user intent is available and `test.agent_after_command` remains `true`. For UI, HTML, CSS, browser, visual layout, or copy-placement changes, the agent attempts reviewer-visible visual evidence and explains in `testing_summary` when screenshots, images, videos, GIFs, or rendered HTML artifacts are not captured.
...
- If GitHub can neither read required-check policy nor report a trustworthy external CI result for the PR head, pauses after the grace period for an explicit operator decision instead of waiting for the whole idle timeout. This is never treated as a pass: approve only after verifying the provider outage, such as an exhausted Actions budget, and recording `external_ci_not_run_budget_exhausted` alongside the locally passing checks for that exact head. It does not bypass branch protection.
