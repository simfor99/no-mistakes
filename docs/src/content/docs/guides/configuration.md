...
- `agent_path_override`, `agent_args_override`, `acpx_path`, `acp_registry_overrides`, `ci_timeout`, `ci_queued_check_attention_after`, `daemon_connect_timeout`, `step_quiet_warning`, `review_no_progress_timeout`, `review_max_duration`, `log_level`, and `session_reuse` are global-only fields.
