...
func TestCIStep_LivePRHeadDriftSuppressesNoChecksPassed(t *testing.T) {
	t.Parallel()
	dir, baseSHA, headSHA := setupGitRepo(t)
	env := append(fakeCIGH(t, "OPEN", "[]"), "FAKE_CLI_LIVE_HEAD=external-head")
	...
	if cimonitor.ChecksPassed(logs) {
		t.Fatalf("head drift must suppress no-checks handoff: %v", logs)
	}
}

func TestCIStep_LivePRHeadDriftSkipsAutoFix(t *testing.T) {
	t.Parallel()
	dir, baseSHA, headSHA := setupGitRepo(t)
	env := append(fakeCIGH(t, "OPEN", `[{"name":"build","state":"FAILURE","bucket":"fail"}]`), "FAKE_CLI_LIVE_HEAD=external-head")
	...
	if len(ag.calls) != 0 {
		t.Fatalf("head drift must skip CI auto-fix calls, got %d", len(ag.calls))
	}
}
