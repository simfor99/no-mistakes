...
- Daemon-restart recovery (`completeTerminalCIRuns` in `internal/daemon/manager.go`, called from `recoverOnStartup` before the parked-gate and fail-closed sweep) completes an orphaned CI monitor only after the provider confirms its PR is merged or closed, and only when the run is PR-backed and otherwise complete (every non-CI step completed or skipped, CI the single running step, worktree still matching the recorded head). Step completion, run completion, and clearing `push_active` happen in one transaction (`db.CompleteRecoveredCIRun`); everything else - open or unknown PR, no recorded PR, probe failure, any other shape - fails closed through the ordinary stale-run sweep. Regressions: `TestRecoverOnStartupReconcilesOnlyTerminalActiveCI`, e2e `TestTerminalCIRecoveryJourney`.
